    ...
    endpoints: (build) => ({
      injected: build.query<{ value: string }, string>({
        ...
        async onCacheEntryAdded(
          ...
          expect(getCacheEntry().dataReducer).toEqual(undefined)
          ...
          expect(getCacheEntry().dataReducer).toEqual({ value: 'success' })
          updateCachedData((draft) => {
            draft.value = 'TEST'
            trackCalls()
          })
          expect(trackCalls).toHaveBeenCalledTimes(1)
          expect(getCacheEntry().dataReducer).toEqual({ value: 'TEST' })
